...
template <typename T>
const T CommandLineParser::getValue(const std::string &name) const {
  throw std::logic_error("Invalid type parameter.");
}
template <>
const std::string CommandLineParser::getValue(const std::string &name) const {
  return values.at(name).value();
}
template <>
const int CommandLineParser::getValue(const std::string &name) const {
  return std::stoi(getValue<std::string>(name));
}

template <typename T>
const T CommandLineParser::optValue(const std::string &name,
...
template <>
const std::string
CommandLineParser::optValue(const std::string &name,
...
template <>
const int CommandLineParser::optValue(const std::string &name,
